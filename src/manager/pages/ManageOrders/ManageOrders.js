            ...
            <ul className="align-right">
              {order.products.map((product) => (
                <li key={product.id}>
                  ...
                  <Button className="custom-button" onClick={() => openConfirmModal(() => handleDeleteProduct(order.id, product.id), "מחיקת מוצר", "האם אתה בטוח שברצונך למחוק מוצר זה?")}>מחיקת מוצר</Button>
                </li>
              ))}
            </ul>
